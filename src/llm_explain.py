...
def build_llm_prompt(text, label_name, confidence, support_rumor_detail, support_non_rumor_detail, decision_score, local_explanation):
    def compact(items):
        if not items:
            return "无明显高贡献特征"
        return "；".join([f"{x['feature']}，贡献值{x['contribution']:.4f}" for x in items])
...
支持“谣言”的词或短语及贡献值：{compact(support_rumor_detail)}
支持“非谣言”的词或短语及贡献值：{compact(support_non_rumor_detail)}
最终决策分数：{decision_score:.4f}
...
def generate_llm_or_fallback_explanation(text, label_name, confidence, support_rumor_detail, support_non_rumor_detail, decision_score, local_explanation, use_llm=False):
    ...
    prompt = build_llm_prompt(text, label_name, confidence, support_rumor_detail, support_non_rumor_detail, decision_score, local_explanation)
